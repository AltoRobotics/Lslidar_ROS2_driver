...
LslidarDriver::LslidarDriver(const rclcpp::NodeOptions &options)
    : Node("lslidar_node", options), last_azimuth(0), is_first_sweep(true),
      sweep_end_time(0.0), return_mode(1), lidar_number_(1), point_num(2000),
      packet_rate(1695.0), current_packet_time(0.0), last_packet_time(0.0),
      horizontal_angle_resolution(0.0), is_get_difop_(false),
      ...
      // ========== DIAGNOSTIC DATA INITIALIZATION ==========
      pps_alignment_angle_(-999.0f), pps_alignment_error_(-999.0f),
      apd_board_temperature_(-999.0f), ld_board_temperature_(-999.0f),
      apd_high_voltage_(-999.0f), ld_high_voltage_(-999.0f),
      plate3_temperature_(-999.0f), plate3_humidity_(-999.0f), gps_status_(0),
      pps_status_(0), high_temp_suspension_(0), cover_dirty_count_(0),
      cover_dirty_alarm_(0), cover_dirty_energy_(0), threshold_adjust_value(0),
      input_voltage_value(-999.0f), input_current_value(-999.0f) {

  return;
}

bool LslidarDriver::checkPacketValidity(
    lslidar_msgs::msg::LslidarPacket::UniquePtr &packet) {
  for (size_t blk_idx = 0; blk_idx < 6; ++blk_idx) {
    if (packet->data[blk_idx * 100] != 0xff &&
        packet->data[blk_idx * 100 + 1] != 0xee) {
      return false;
    }
  }
  return true;
...
bool LslidarDriver::loadParameters() {
  ...
  this->get_parameter("horizontal_angle_resolution",
                      horizontal_angle_resolution);
...
void LslidarDriver::outputParameters() {
  ...
  LS_PARAM << "horizontal_angle_resolution: " << horizontal_angle_resolution
           << LS_END;
  ...
  if (publish_scan)
    LS_INFO << "scan_num: " << scan_num << LS_END;

  LS_SOCKET << "Accepting packets from IP address: " << lidar_ip_string.c_str()
            << LS_END;
  if (add_multicast)
    LS_INFO << "Adding multicast groupaddress: " << group_ip_string.c_str()
            << LS_END;
...
bool LslidarDriver::createRosIO() {
  ...
  diagnostic_pub = this->create_publisher<lslidar_msgs::msg::LslidarDiagnostic>(
      "lslidar_diagnostics", 10);

  rmw_qos_profile_t qos_profile = rmw_qos_profile_default;
  lslidar_control_service_ =
      this->create_service<lslidar_msgs::srv::LslidarControl>(
          "lslidar_control",
          std::bind(&LslidarDriver::powerOn, this, std::placeholders::_1,
                    std::placeholders::_2),
          qos_profile);
  motor_control_service_ =
      this->create_service<lslidar_msgs::srv::MotorControl>(
          "motor_control",
          std::bind(&LslidarDriver::motorControl, this, std::placeholders::_1,
                    std::placeholders::_2),
          qos_profile);
  motor_speed_service_ = this->create_service<lslidar_msgs::srv::MotorSpeed>(
      "set_motor_speed",
      std::bind(&LslidarDriver::motorSpeed, this, std::placeholders::_1,
                std::placeholders::_2),
      qos_profile);
  data_port_service_ = this->create_service<lslidar_msgs::srv::DataPort>(
      "set_data_port",
      std::bind(&LslidarDriver::setDataPort, this, std::placeholders::_1,
                std::placeholders::_2),
      qos_profile);
  dev_port_service_ = this->create_service<lslidar_msgs::srv::DevPort>(
      "set_dev_port",
      std::bind(&LslidarDriver::setDevPort, this, std::placeholders::_1,
                std::placeholders::_2),
      qos_profile);
  data_ip_service_ = this->create_service<lslidar_msgs::srv::DataIp>(
      "set_data_ip",
      std::bind(&LslidarDriver::setDataIp, this, std::placeholders::_1,
                std::placeholders::_2),
      qos_profile);
  destination_ip_service_ =
      this->create_service<lslidar_msgs::srv::DestinationIp>(
          "set_destination_ip",
          std::bind(&LslidarDriver::setDestinationIp, this,
                    std::placeholders::_1, std::placeholders::_2),
          qos_profile);
  time_service_ = this->create_service<lslidar_msgs::srv::TimeService>(
      "time_service",
      std::bind(&LslidarDriver::timeService, this, std::placeholders::_1,
                std::placeholders::_2),
      qos_profile);

  if (dump_file != "") {
    msop_input_.reset(new lslidar_driver::InputPCAP(this, msop_udp_port, 1212,
                                                    packet_rate, dump_file));
    difop_input_.reset(new lslidar_driver::InputPCAP(this, difop_udp_port, 1206,
                                                     1, dump_file));
  } else {
    msop_input_.reset(
        new lslidar_driver::InputSocket(this, msop_udp_port, 1212));
    difop_input_.reset(
        new lslidar_driver::InputSocket(this, difop_udp_port, 1206));
  }
...
bool LslidarDriver::initialize() {
  ...
  if (publish_scan) {
    ...
    point_size = ceil((scan_msg->angle_max - scan_msg->angle_min) /
                      scan_msg->angle_increment);
    scan_msg->ranges.assign(point_size,
                            std::numeric_limits<float>::quiet_NaN());
    scan_msg->intensities.assign(point_size,
                                 std::numeric_limits<float>::quiet_NaN());
  }
...
void LslidarDriver::parseDiagnosticData(
  ...
  // ===== COVER DIRTY (Offset 96-100) =====
  cover_dirty_count_ = difop_packet->data[96];
  cover_dirty_alarm_ = difop_packet->data[97];
  cover_dirty_energy_ = (difop_packet->data[98] << 8) | difop_packet->data[99];
  threshold_adjust_value = difop_packet->data[100];
}

void LslidarDriver::publishDiagnostics() {

  auto diag_msg = lslidar_msgs::msg::LslidarDiagnostic();

  // Header con timestamp corrente
  ...
  // Temperature
  diag_msg.apd_board_temperature = apd_board_temperature_;
  diag_msg.ld_board_temperature = ld_board_temperature_;
  diag_msg.plate3_temperature = plate3_temperature_;
  ...
  // Current
  diag_msg.input_current = input_current_value;

  // Status flags
  diag_msg.gps_status = gps_status_;
  diag_msg.pps_status = pps_status_;
  diag_msg.high_temp_suspension = high_temp_suspension_;

  // Cover dirty
  diag_msg.cover_dirty_count = cover_dirty_count_;
  diag_msg.cover_dirty_alarm = cover_dirty_alarm_;
  diag_msg.cover_dirty_energy = cover_dirty_energy_;
  diag_msg.threshold_adjust_value = threshold_adjust_value;

  // Pubblica il messaggio diagnostico
  diagnostic_pub->publish(diag_msg);
}

void LslidarDriver::difopPoll() {
  // reading and publishing scans as fast as possible.
  lslidar_msgs::msg::LslidarPacket::UniquePtr difop_packet_ptr(
  ...
  while (rclcpp::ok()) {
    ...
    if (rc == 0) {
      if (difop_packet_ptr->data[0] != 0xa5 ||
          difop_packet_ptr->data[1] != 0xff ||
          difop_packet_ptr->data[2] != 0x00 ||
          difop_packet_ptr->data[3] != 0x5a) {
        return;
      }
      ...
      // Parse e pubblica dati diagnostici
      parseDiagnosticData(difop_packet_ptr);

      publishDiagnostics();

      if (difop_packet_ptr->data[1196] == 0x03 ||
          (difop_packet_ptr->data[1196] == 0x02 &&
           difop_packet_ptr->data[1197] / 16 == 8)) {
        fpga_type = 3;
        for (int i = 0; i < 32; ++i) {
        ...
        int angle_a0 =
            difop_packet_ptr->data[186] * 256 + difop_packet_ptr->data[187];
        adjust_angle[0] = angle_a0 > 32767 ? 32767 - angle_a0 : angle_a0;

        int angle_a1 =
            difop_packet_ptr->data[190] * 256 + difop_packet_ptr->data[191];
        adjust_angle[1] = angle_a1 > 32767 ? 32767 - angle_a1 : angle_a1;

        int angle_a2 =
            difop_packet_ptr->data[188] * 256 + difop_packet_ptr->data[189];
        adjust_angle[2] = angle_a2 > 32767 ? 32767 - angle_a2 : angle_a2;

        int angle_a3 =
            difop_packet_ptr->data[192] * 256 + difop_packet_ptr->data[193];
      ...
      } else if (difop_packet_ptr->data[1198] == 0x07 ||
                 (difop_packet_ptr->data[1198] & 0x0F) == 4) {
        fpga_type = 4;
        time_service_mode_ = difop_packet_ptr->data[45];
        if (is_msc16 && difop_packet_ptr->data[1198] / 16 == 7 &&
            difop_packet_ptr->data[1202] / 16 == 7) {
          for (int j = 0; j < 16; ++j) {
            int adjust_angle_ = difop_packet_ptr->data[680 + 2 * j] * 256 +
                                difop_packet_ptr->data[681 + 2 * j];
            adjust_angle_ =
                adjust_angle_ > 32767 ? adjust_angle_ - 65535 : adjust_angle_;
            msc16_adjust_angle[j] = static_cast<float>(adjust_angle_) * 0.01f;

            int offset_angle = difop_packet_ptr->data[712 + 2 * j] * 256 +
                               difop_packet_ptr->data[713 + 2 * j];
            offset_angle =
                offset_angle > 32767 ? offset_angle - 65535 : offset_angle;
            msc16_offset_angle[j] = static_cast<float>(offset_angle) * 0.01f;
            if (fabs(msc16_adjust_angle[j] -
                     c16_vertical_angle[c16_remap_angle[j]]) > 1.5) {
        ...
        if (is_print_working_time) {
          int total_working_time = (difop_packet_ptr->data[105] << 24) +
                                   (difop_packet_ptr->data[106] << 16) +
                                   (difop_packet_ptr->data[107] << 8) +
                                   difop_packet_ptr->data[108];
          int less_minus40_degree_working_time =
              (difop_packet_ptr->data[112] << 16) +
              (difop_packet_ptr->data[113] << 8) + difop_packet_ptr->data[114];
          int minus40_to_minus10_working_time =
              (difop_packet_ptr->data[115] << 16) +
              (difop_packet_ptr->data[116] << 8) + difop_packet_ptr->data[117];
          int minus10_to_30_working_time = (difop_packet_ptr->data[118] << 16) +
                                           (difop_packet_ptr->data[119] << 8) +
                                           difop_packet_ptr->data[120];
          int positive30_to_70_working_time =
              (difop_packet_ptr->data[121] << 16) +
              (difop_packet_ptr->data[122] << 8) + difop_packet_ptr->data[123];
          int positive70_to_100_working_time =
              (difop_packet_ptr->data[124] << 16) +
              (difop_packet_ptr->data[125] << 8) + difop_packet_ptr->data[126];

          LS_INFO << "total working time: " << total_working_time / 60
                  << " hours: " << total_working_time % 60 << " minutes."
                  << LS_END;
          LS_INFO << "less than     -40 degrees  working time: "
                  << less_minus40_degree_working_time / 60
                  << " hours: " << less_minus40_degree_working_time % 60
                  << "minutes" << LS_END;
          LS_INFO << "-40 degrees ~ -10 degrees  working time: "
                  << minus40_to_minus10_working_time / 60
                  << " hours: " << minus40_to_minus10_working_time % 60
                  << "minutes" << LS_END;
          LS_INFO << "-10 degrees ~ 30  degrees  working time: "
                  << minus10_to_30_working_time / 60
                  << " hours: " << minus10_to_30_working_time % 60 << "minutes"
                  << LS_END;
          LS_INFO << "30  degrees ~ 70  degrees  working time: "
                  << positive30_to_70_working_time / 60
                  << " hours: " << positive30_to_70_working_time % 60
                  << "minutes" << LS_END;
          LS_INFO << "70  degrees ~ 100 degrees  working time: "
                  << positive70_to_100_working_time / 60
                  << " hours: " << positive70_to_100_working_time % 60
                  << "minutes" << LS_END;
...
void LslidarDriver::pointcloudToLaserscan(
  ...
  uint32_t ranges_size =
      std::ceil((output_scan.angle_max - output_scan.angle_min) /
                output_scan.angle_increment);

  // determine if laserscan rays with no obstacle data will evaluate to infinity
  // or max_range
  output_scan.ranges.assign(ranges_size,
                            std::numeric_limits<double>::infinity());
  output_scan.intensities.assign(ranges_size,
                                 std::numeric_limits<float>::quiet_NaN());

  // Iterate through pointcloud
  for (sensor_msgs::PointCloud2ConstIterator<float> iter_x(cloud_msg, "x"),
       iter_y(cloud_msg, "y"), iter_z(cloud_msg, "z"),
       iter_intensity(cloud_msg, "intensity");
    ...
    if (range < min_range) {
      RCLCPP_DEBUG(
          ...
          range, min_range, *iter_x, *iter_y, *iter_z);
      continue;
    }
    if (range > max_range) {
      RCLCPP_DEBUG(
          ...
          range, max_range, *iter_x, *iter_y, *iter_z);
      continue;
    }
...
void LslidarDriver::decodePacket(
  ...
  for (size_t block_idx = 0; block_idx < BLOCKS_PER_PACKET; ++block_idx) {
    ...
    // 32 scan
    for (size_t scan_fir_idx = 0; scan_fir_idx < SCANS_PER_FIRING_CX;
         ++scan_fir_idx) {
      ...
      // distance
      firings.distance[block_idx * 32 + scan_fir_idx] = static_cast<float>(
          (packet->data[block_idx * 100 + byte_idx + 4] +
           (packet->data[block_idx * 100 + byte_idx + 5] << 8)) *
          DISTANCE_RESOLUTION * distance_unit);

      // intensity
      firings.intensity[block_idx * 32 + scan_fir_idx] =
          static_cast<float>(packet->data[block_idx * 100 + byte_idx + 6]);
    }
  }
...
bool LslidarDriver::poll() {
  ...
  if (use_time_service && 4 == fpga_type) {
    if (time_service_mode_ == 1) { // ptp授时
      uint32_t timestamp_s =
          packet->data[1201] * pow(2, 32) + packet->data[1202] * pow(2, 24) +
      ...
      packet_time_ns = packet->data[1206] + packet->data[1207] * pow(2, 8) +
                       packet->data[1208] * pow(2, 16) +
                       packet->data[1209] * pow(2, 24); // ns
      ...
      packet_time_s = static_cast<uint64_t>(timegm(&cur_time)); // s
      packet_time_ns =
          (packet->data[1206] + packet->data[1207] * pow(2, 8) +
           packet->data[1208] * pow(2, 16) + packet->data[1209] * pow(2, 24)) *
          1e3; // ns
      timeStamp = rclcpp::Time(packet_time_s, packet_time_ns);
      packet->stamp = timeStamp;
      current_packet_time = timeStamp.seconds();
    ...
    packet_time_s = static_cast<uint64_t>(timegm(&cur_time)); // s
    packet_time_ns =
        (packet->data[1200] + packet->data[1201] * pow(2, 8) +
         packet->data[1202] * pow(2, 16) + packet->data[1203] * pow(2, 24)) *
        1e3; // ns
    timeStamp = rclcpp::Time(packet_time_s, packet_time_ns);

    if ((timeStamp - timeStamp_bak).seconds() < 0.0 &&
        (timeStamp - timeStamp_bak).seconds() > -1.0 &&
        packet_time_ns < 100000000) {
      timeStamp = rclcpp::Time(packet_time_s + 1, packet_time_ns);
    } else if ((timeStamp - timeStamp_bak).seconds() > 1.0 &&
               (timeStamp - timeStamp_bak).seconds() < 1.2 &&
  ...
  if (lidar_type == "c32_3") {
    for (int blk_idx = 0; blk_idx < BLOCKS_PER_PACKET; ++blk_idx) {
      for (int scan_fir_idx = 0; scan_fir_idx < SCANS_PER_BLOCK;
           ++scan_fir_idx) {
        if (1 >= scan_fir_idx % 4) {
  ...
  for (size_t fir_idx = start_fir_idx; fir_idx < end_fir_idx; ++fir_idx) {
    if (angle_change) {
      ...
      if ("MSC16" == lidar_type) {
        firings.azimuth[fir_idx] +=
            msc16_offset_angle[fir_idx % 16] < 0
                ? msc16_offset_angle[fir_idx % 16] + 36000
                : msc16_offset_angle[fir_idx % 16];
        if (firings.azimuth[fir_idx] >= 36000) {
          firings.azimuth[fir_idx] -= 36000;
        }
      }
    ...
    if (!(firings.distance[fir_idx] >= min_range &&
          firings.distance[fir_idx] <= max_range))
      continue;
    if (angle_able_max > 36000) {
      if ((firings.azimuth[fir_idx] > (angle_able_max - 36000)) &&
          firings.azimuth[fir_idx] < angle_able_min)
        continue;
    } else {
      if ((firings.azimuth[fir_idx] > angle_able_max) ||
          firings.azimuth[fir_idx] < angle_able_min)
        continue;
    }
    ...
    if (coordinate_opt) {
      int tmp_idx = conversionAngle - firings.azimuth[fir_idx] < 0
                        ? conversionAngle - firings.azimuth[fir_idx] + 36000
                        : conversionAngle - firings.azimuth[fir_idx];
      ...
      int tmp_idx = firings.azimuth[fir_idx] - conversionAngle < 0
                        ? firings.azimuth[fir_idx] - conversionAngle + 36000
                        : firings.azimuth[fir_idx] - conversionAngle;
    ...
    if (last_packet_time > 1e-6) {
      time = current_packet_time - sweep_end_time -
             (current_packet_time - last_packet_time) *
    ...
    case 32:
      ...
      break;
    case 31:
      remapped_scan_idx = (fir_idx % 32) % 2 == 0 ? (fir_idx % 32) / 2
                                                  : (fir_idx % 32) / 2 + 16;
      break;
    case 17:
      remapped_scan_idx =
          (fir_idx % 16) / 4 + (fir_idx % 16) % 4 * 4; // c16 4.0 国产化
      break;
    case 16:
      remapped_scan_idx =
          (fir_idx % 16) % 2 == 0 ? (fir_idx % 16) / 2 : (fir_idx % 16) / 2 + 8;
      break;
    case 8:
      ...
      if (scan_num == remapped_scan_idx) {
        if (coordinate_opt) {
          float horizontal_angle =
              firings.azimuth[fir_idx] * 0.01f * DEG_TO_RAD;
          uint point_index = (int)((horizontal_angle - scan_msg->angle_min) /
                                   scan_msg->angle_increment);
          point_index = (point_index < point_size) ? point_index
                                                   : (point_index % point_size);
          scan_msg->ranges[point_size - point_index - 1] =
              firings.distance[fir_idx];
          scan_msg->intensities[point_size - point_index - 1] =
              firings.intensity[fir_idx];
        } else {
          float h_angle = (45000.0 - firings.azimuth[fir_idx]) < 36000.0
                              ? 45000.0 - firings.azimuth[fir_idx]
                              : 9000.0 - firings.azimuth[fir_idx];
          float horizontal_angle = h_angle * 0.01f * DEG_TO_RAD;

          // float horizontal_angle = firings.azimuth[fir_idx] * 0.01f *
          // DEG_TO_RAD;
          uint point_index = (int)((horizontal_angle - scan_msg->angle_min) /
                                   scan_msg->angle_increment);
          point_index = (point_index < point_size) ? point_index
  ...
  if (end_fir_idx != SCANS_PER_PACKET) {
    ...
    // sweep_end_time = packet->stamp.toSec() - (end_fir_idx - start_fir_idx)
    // * 3.125 * 1e-6;
    ...
    for (size_t fir_idx = start_fir_idx; fir_idx < end_fir_idx; ++fir_idx) {
      ...
      if (last_packet_time > 1e-6) {
        ...
        // *
        //        (SCANS_PER_PACKET - fir_idx - 1) / SCANS_PER_PACKET;
        time = (current_packet_time - last_packet_time) *
               (fir_idx - start_fir_idx) / SCANS_PER_PACKET;
      } else {
        time = current_packet_time;
      }
      ...
      case 17:
        ...
        break;
      case 16:
        remapped_scan_idx = (fir_idx % 16) % 2 == 0 ? (fir_idx % 16) / 2
                                                    : (fir_idx % 16) / 2 + 8;
        break;
      case 8:
        remapped_scan_idx = (fir_idx % 8) % 2 * 4 + (fir_idx % 8) / 2;
      ...
      if (publish_scan) {
        if (scan_num == remapped_scan_idx) {
          if (coordinate_opt) {
            ...
            uint point_index = (int)((horizontal_angle - scan_msg->angle_min) /
                                     scan_msg->angle_increment);
            point_index = (point_index < point_size)
                              ? point_index
                              : (point_index % point_size);
            scan_msg->ranges[point_size - point_index - 1] =
                firings.distance[fir_idx];
...
bool LslidarDriver::sendPacketTolidar(unsigned char *config_data) const {
  ...
  sendto(socketid, (const char *)config_data, 1206, 0,
         (struct sockaddr *)&addrSrv, sizeof(addrSrv));
  return true;
}

bool LslidarDriver::powerOn(
    std::shared_ptr<lslidar_msgs::srv::LslidarControl::Request> req,
    std::shared_ptr<lslidar_msgs::srv::LslidarControl::Response> res) {
  lslidar_msgs::msg::LslidarPacket::UniquePtr packet0(
      new lslidar_msgs::msg::LslidarPacket());
  packet0->data[0] = 0x00;
  packet0->data[1] = 0x00;
  int rc_msop = -1;
  ...
  if (config_data[0] != 0xa5 || config_data[1] != 0xff ||
      config_data[2] != 0x00 || config_data[3] != 0x5a) {
    res->result = 0;
    LS_ERROR << "Can not get dev packet! Set failed!" << LS_END;
    return true;
  }
  ...
  if (req->laser_control == 1) {
    if ((rc_msop = msop_input_->getPacket(packet0)) == 0) {
      res->result = 1;
      LS_WARN << "receive cmd: " << req->laser_control
              << " already power on status" << LS_END;
      return true;
    }
    ...
    } while ((rc_msop != 0) && (packet0->data[0] != 0xff) &&
             (packet0->data[1] != 0xee));
    sleep(3);
    res->result = 1;
...
bool LslidarDriver::timeService(
    std::shared_ptr<lslidar_msgs::srv::TimeService::Request> req,
  ...
  transform(time_service_mode.begin(), time_service_mode.end(),
            time_service_mode.begin(), ::tolower);
  ...
  if (time_service_mode == "gps") {
    ...
    std::regex ipv4("\\b(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\\.){3}(?:"
                    "25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\\b");
    if (!regex_match(ntp_ip, ipv4)) {
      LS_ERROR << "Parameter error, please check the input parameters"
               << LS_END;
      res->result = false;
      return true;
    }
    unsigned short first_value, second_value, third_value, end_value;
    sscanf(ntp_ip.c_str(), "%hu.%hu.%hu.%hu", &first_value, &second_value,
           &third_value, &end_value);
  ...
  res->result = true;
  sendPacketTolidar(config_data);
  LS_MSG << "The current timing method is " << time_service_mode
         << ". Time service method modified successfully!" << LS_END;
  ;
  return true;
}

bool LslidarDriver::motorControl(
    std::shared_ptr<lslidar_msgs::srv::MotorControl::Request> req,
  ...
  res->result = true;
  sendPacketTolidar(config_data);
  LS_MSG << "Motor control: " << req->motor_control << ", set successfully!"
         << LS_END;
  return true;
}

bool LslidarDriver::motorSpeed(
    std::shared_ptr<lslidar_msgs::srv::MotorSpeed::Request> req,
  ...
  res->result = true;
  sendPacketTolidar(config_data);
  LS_MSG << "Motor speed: " << req->motor_speed << " HZ, set successfully!"
         << LS_END;
  return true;
}

bool LslidarDriver::setDataPort(
    std::shared_ptr<lslidar_msgs::srv::DataPort::Request> req,
  ...
  if (req->data_port < 1025 || req->data_port > 65535 ||
      req->data_port == dev_port) {
    LS_ERROR << "Parameter error, please check the input parameters" << LS_END;
    res->result = false;
    return true;
  } else {
  ...
  res->result = true;
  sendPacketTolidar(config_data);
  LS_MSG << "Set successfully! Current lidar MSOP port:" << req->data_port
         << LS_END;
  LS_MSG << "Please modify the corresponding parameters in the launch file"
         << LS_END;
  return true;
}

bool LslidarDriver::setDevPort(
    std::shared_ptr<lslidar_msgs::srv::DevPort::Request> req,
  ...
  if (req->dev_port < 1025 || req->dev_port > 65535 ||
      req->dev_port == data_port) {
    LS_ERROR << "Parameter error, please check the input parameters" << LS_END;
    res->result = false;
    return true;
  } else {
  ...
  res->result = true;
  sendPacketTolidar(config_data);
  LS_MSG << "Set successfully! Current lidar DIFOP port:" << req->dev_port
         << LS_END;
  LS_MSG << "Please modify the corresponding parameters in the launch file"
         << LS_END;
  return true;
...
bool LslidarDriver::setDataIp(
    std::shared_ptr<lslidar_msgs::srv::DataIp::Request> req,
    std::shared_ptr<lslidar_msgs::srv::DataIp::Response> res) {
  std::regex ipv4("\\b(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\\.){3}(?:25["
  ...
  unsigned short first_value, second_value, third_value, end_value;
  sscanf(req->data_ip.c_str(), "%hu.%hu.%hu.%hu", &first_value, &second_value,
         &third_value, &end_value);

  std::string destination_ip = std::to_string(config_data[14]) + "." +
                               std::to_string(config_data[15]) + "." +
                               std::to_string(config_data[16]) + "." +
                               std::to_string(config_data[17]);
  if (first_value == 0 || first_value == 127 ||
      (first_value >= 224 && first_value <= 255) ||
      destination_ip == req->data_ip) {
    LS_ERROR << "Parameter error, please check the input parameters" << LS_END;
    res->result = false;
    return true;
  } else {
  ...
  res->result = true;
  sendPacketTolidar(config_data);
  LS_MSG << "Set successfully! Current lidar ip:" << req->data_ip.c_str()
         << LS_END;
  LS_MSG << "Please modify the corresponding parameters in the launch file"
         << LS_END;
  return true;
...
bool LslidarDriver::setDestinationIp(
    std::shared_ptr<lslidar_msgs::srv::DestinationIp::Request> req,
  ...
  unsigned short first_value, second_value, third_value, end_value;
  sscanf(req->destination_ip.c_str(), "%hu.%hu.%hu.%hu", &first_value,
         &second_value, &third_value, &end_value);

  std::string data_ip = std::to_string(config_data[10]) + "." +
                        std::to_string(config_data[11]) + "." +
                        std::to_string(config_data[12]) + "." +
                        std::to_string(config_data[13]);
  if (first_value == 0 || first_value == 127 ||
      (first_value >= 240 && first_value <= 255) ||
      data_ip == req->destination_ip) {
    LS_ERROR << "Parameter error, please check the input parameters" << LS_END;
    res->result = false;
    return true;
  } else {
  ...
  res->result = true;
  sendPacketTolidar(config_data);
  LS_MSG << "Set successfully! Current lidar destination ip: "
         << req->destination_ip.c_str() << LS_END;
  LS_MSG << "Please modify the local IP address to "
         << req->destination_ip.c_str() << LS_END;
  return true;
}

bool LslidarDriver::determineLidarType() {
  lslidar_msgs::msg::LslidarPacket::UniquePtr pkt(
      new lslidar_msgs::msg::LslidarPacket);
  ...
  if (pkt->data[1211] == 0x01) {
    ...
    if (pkt->data[1204] == 0x39)
      return_mode = 2;
  } else if (pkt->data[1211] == 0x00 && pkt->data[1205] == 0x20) {
    for (int j = 0; j < 32; ++j) {
      config_vertical_angle_tmp[j] =
          config_vertical_angle_32[adjust_angle_index[j]];
